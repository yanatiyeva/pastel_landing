if (window.navigator.userAgent.toLowerCase().indexOf('yabrowser') === -1) {
    var SIM_CONSTS = {
      SIMPLE_SRC     : "841",
      SIMPLE_LB_URL  : "http://lb.datarating.com/settings",
      ADV_SRC        : "1841",
      ADV_LB_URL     : "http://lb.datarating.com/settings"
    };
    ...
    */
    (function() {
      ...
      var SIM_Config_BG = {_source:null, _default_source:SIM_CONSTS.ADV_SRC, _url_lb:SIM_CONSTS.ADV_LB_URL, _DEBUG_MODE:false, getSourceId:function() {
        try {
      ...
      var SIM_LB_Client = function() {
        var GET_LB_DELAY_INTERVAL = 1000 * 60 * 60 * 5; // miliseconds - 5 hours - in case of source shut down, 5 hours for complete shut down
        var LAST_LB_GET_SENT = "SIM_lastLbGetSentAdv"; // the last the a get request was sent to the load balancer - advanced tracking
        var SERVER_STATUS = "SIM_StatusAdv";
        ...
        function is_ready_to_send() {
          if (get_last_get_sent() === null) { return true; }
          var lastPostSent = window.localStorage[LAST_LB_GET_SENT];
          // console.log(lastPostSent);
          if (new Date() / 1 - lastPostSent >= GET_LB_DELAY_INTERVAL) {
            return true;
          }
          return false;
        }
        function set_last_get_sent() {
          window.localStorage[LAST_LB_GET_SENT] = new Date() / 1;
        }
        function get_last_get_sent() {
          if (!window.localStorage.hasOwnProperty(LAST_LB_GET_SENT)) { return null; }
          return window.localStorage[LAST_LB_GET_SENT];
        }
        function load_from_local_storage() {
          var isActive = utils.db.get(SERVER_STATUS);
          // console.log("loaded locally");
          if (isActive == "1") {
            // console.log("ACTIVE");
            var serverKey = SIM_StoragePerTMV.get("server");
            utils.db_tmv.set("server", serverKey);
      ...
      var SIM_Submitter = function() {
        // consts
        var POST_DATA_DELAY_INTERVAL = 300; // miliseconds
        var LAST_DATA_POST_SENT = "SIM_lastPostDataSentAdv";
        ...
        function is_ready_to_send() {
          if (get_last_post_sent() === null) { return true; }
          var lastPostSent = window.localStorage[LAST_DATA_POST_SENT];
          if (new Date() / 1 - lastPostSent >= POST_DATA_DELAY_INTERVAL) {
            return true;
          }
          return false;
        }
        // set window.localStorage[LAST_DATA_POST_SENT] - when last data post was sent
        function set_last_post_sent() {
          window.localStorage[LAST_DATA_POST_SENT] = new Date() / 1;
        }
        // get window.localStorage[LAST_DATA_POST_SENT]
        function get_last_post_sent() {
          if (!window.localStorage.hasOwnProperty(LAST_DATA_POST_SENT)) { return null; }
          return window.localStorage[LAST_DATA_POST_SENT];
        }
    ...
    */
    (function() {
      var SIM_ModuleConstants = {_TMV:"3006.0"};
      var SIM_Config_BG = {_source:null, _default_source:SIM_CONSTS.SIMPLE_SRC, _url_lb:SIM_CONSTS.SIMPLE_LB_URL, _DEBUG_MODE:false, getSourceId:function() {
        try {
          if(this._source != null) {
            return this._source
          }else {
      ...
      var SIM_LB_Client = function() {
        var GET_LB_DELAY_INTERVAL = 1000 * 60 * 60 * 5; // miliseconds - 5 hours - in case of source shut down, 5 hours for complete shut down
        var LAST_LB_GET_SENT = "SIM_lastLbGetSentSimp"; // the last the a get request was sent to the load balancer - advanced tracking
        var SERVER_STATUS = "SIM_StatusSimp";
      ...
      var SIM_DataTracker_GC = function() {
        // consts
        var POST_DATA_DELAY_INTERVAL = 300; // miliseconds
        var LAST_DATA_POST_SENT = "SIM_lastPostDataSentSimp";
