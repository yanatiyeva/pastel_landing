...
<html>
   ...
   <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <body>
        <p>Новая заявка с сайта http://go.decormehedova.ru/landing/kursi-decora-ukraine на участие в тренинге:</p>
        <p>Имя: '.htmlspecialchars($_REQUEST['name']).'</p>
		<p>E-mail: '. htmlspecialchars($_REQUEST['email']).'</p>   
        <p>Телефон: '. $_REQUEST['phone'].'</p>
        <p>Заказанный пакет услуг:'.htmlspecialchars($_REQUEST['standart']).' '. htmlspecialchars($_REQUEST['gold']).' ' .htmlspecialchars($_REQUEST['vip']) .'</p>
    </body>
</html>';
